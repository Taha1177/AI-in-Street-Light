...
void setup() {
  ...
  Serial.println("BLYNK THRESHOLD ADAPTIVE LIGHTING STARTED");
}
...
void loop() {
  ...
  if (now - lastControlTime >= CONTROL_INTERVAL) {
    ...
    if (lastLDR < 500) {
      lastPredictedClass = 0;
    } else if (lastLDR <= 800) {
      lastPredictedClass = 1;
    } else {
      lastPredictedClass = 2;
    }
    ...
    Serial.print(" THRESH="); Serial.print(classNames[lastPredictedClass]);
